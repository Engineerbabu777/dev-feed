  ...
  const unactiveNav = 'flex gap-3 py-2 my-2 transition-all hover:scale-110 hover:bg-blue-300 hover:bg-opacity-30 -mx-4 px-4 hover:shadow-gray-300 hover:shadow-md ';
    ...
    return(<>
         <Card>
          ...
          <Link href="/" className={router.pathname === '/' ? activeNav : unactiveNav}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
              <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
            </svg>Home</Link>
          <Link href="/friends" className={router.pathname.split('/').includes('friends') ? activeNav : unactiveNav}>
          ...
          <Link href="/saved" className={router.pathname.split('/').includes('saved') ? activeNav : unactiveNav}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
              <path stroke-linecap="round" stroke-linejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0z" />
            </svg>Saved Posts</Link>
          <Link href="/notifications" className={router.pathname.split('/').includes('notifications') ? activeNav : unactiveNav}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
